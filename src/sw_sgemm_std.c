...
extern void SLAVE_FUN(sgemm_dma_trans)();
extern void SLAVE_FUN(copy_border_float32)();
extern void SLAVE_FUN(copy_border_back_float32)();
                 ...
                 const float beta, float *C, const int ldc)
{
  ...
  if(TransB == CblasNoTrans ) {
    ...
    if(TransA == CblasNoTrans)
      ...
      athread_spawn(sgemm_dma_trans, params);
    athread_join();
#ifdef DEBUG_VERBOSE
    gettimeofday(&t2, NULL);
...
#endif
    athread_spawn(copy_border_back_float32, cd);
    athread_join();
    /*if(Ne > Ns)
    {
      for(j = Ns; j < RN; j ++)
