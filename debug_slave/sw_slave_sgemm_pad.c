 ...
 * On good if Water
 *
 * input (Trans)  is of dim(K, M)
 * weight is of dim(K, N)
 * ouput (Trans) is of dim(N, M)
 * blkM is 128x, blkK is 8x, blkN is 32x
 * ************/

#define LDM_ALINGN_SIZE 256
void FJR_blas_sgemm_trans_pad(ConvData* param)
{
  ...
  int N = param->N;
  int blkN = param->blkN;
  int numN = (N + blkN - 1)/blkN;
  int M = param->M;
  int blkM = param->blkM;
  int T = (M + blkM - 1)/blkM;
  int K = param->K;
  int numK = (K + blkK - 1)/blkK;
//#define _LDM_ALIGN_
...
/*
//M, K, Ci, Ri
  doublev4* local_input_start  = (doublev4*)ldm_malloc(sizeof(double)*blkK*blkM/8/8*2);
  double* local_input = (double*)local_input_start;// + 16 - (long)local_input_start/8%128/8;
  int local_input_size = blkK*blkM/8/8;
//N, K, K, K
  doublev4* local_weight_start = (doublev4*)ldm_malloc(sizeof(double)*blkK*blkN/8/8*2);
  double* local_weight = (double*)local_weight_start;// + 16 - (long)local_weight_start/8%128/8;
  int local_weight_size = blkK*blkN/64;
//M, N, Co, Ro
  doublev4* local_output_start = (doublev4*) ldm_malloc(sizeof(double)*blkN*blkM/8/8);
  double* local_output = (double*)local_output_start;// + 16 - (long)local_output_start/8%128/8;
  int local_output_size = blkN*blkM/8/8;
  */
...
//  rtc_start = slave_get_rtc();
  ...
  if ( id == 63 ) {
    printf("rpc_sgemm_trans gflops %lf \n", (double)M*N*blkK*numK*2/1024/1024/1024/t);
  }
#endif
