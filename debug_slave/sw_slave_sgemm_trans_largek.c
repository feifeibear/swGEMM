 ...
 * ************/
#define SIMDSIZE 4
void FJR_blas_sgemm_trans_largek(ConvData* param)
{
...
//  rtc_start = slave_get_rtc();
  ...
  dma(dma_get_weight, (long)((float*)param->weight+(cid*K*N/8 + rid*N/8)), (long)(local_weight));
  ...
  float* input_start = (float*)param->input + rid*K*M*T/8 + cid*M/8;
  float* output_start = (float*)param->output + rid*N*M*T/8 + cid*M/8;
...
//TODO: BUG,not compatible with -O2 compile flag
    ...
    dma(dma_put_output, (long)((float*)output_start + cT*M), (long)(local_output + cur_output_idx*cur_output_offset));
