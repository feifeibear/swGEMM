...
void dgemm_dma(ConvData* param)
{
    ...
    double* startB = (double*)param->weight + rid*bK/8 + cid*bN*K/8;
    double* startC = (double*)param->output + rid*bM/8 + cid*bN*M/8;
    double* startAp = (double*)param->inputp + rid*bM/8 + cid*bK*Me/8;
    double* startBp = (double*)param->weightp + rid*bK/8 + cid*bN*Ke/8;
    ...
    dma_set_bsize( &dmagetB, ((bK/8)*sizeof(double)) ); 
    dma_set_stepsize( &dmagetB, ((K-(bK/8))*sizeof(double)) ); 
